const ComponentBlock = () => {
    return $('<div>').addClass('doc_content_wrapper').append(
        ...
        text('Напротив, когда мы говорим о реализации Компонента, мы не способны даже организовать его работу - вне модуля. Давайте сразу посмотрим на создание структуры простого компонента.'),
...
$(Example, 'example_key');`),
        ...
        text('А теперь подробнее рассмотрим структуру которую мы тут видим.'),
...
class Main extends MC {
    ...
    render(state, props) {
        ...
        text('Почти всё что тут было применено, было обсуждено ранее за исключением 2 вещей. Первое - это создание контроллера в компоненте, методом создания глобального контроллера'),
        text('*ПРОВЕРЬ РАБОТУ PROPS ДЛЯ ДОЧЕРНИХ КОМПОНЕНТОВ И ОПИШИ, ПРИМЕР ВЫШЕ*'),
        text('Второе, это передача компонента внутрь компонента, это и будет нашей структурой компонентов. Как правило вы будете собирать вашу страницу из таких компонентов, создавая при этом цельную структуру, которую будет гораздо легче поддерживать. Давайте посмотрим, что в нашем Person:'),
