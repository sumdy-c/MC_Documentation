...
class Main extends MC {
    ...
    render(state, props) {
        ...
        text('*Всё работает, нужно описать как работает обновление пропсов и зачем мы пихнули туда глобал стейт перепиши клики на это  () => this.update())*'),
        text('Второе, это передача компонента внутрь компонента, это и будет нашей структурой компонентов. Как правило вы будете собирать вашу страницу из таких компонентов, создавая при этом цельную структуру, которую будет гораздо проще поддерживать. Давайте посмотрим, что в нашем Person:'),
