const InstallBlock = () => {
    return $('<div>').addClass('doc_content_wrapper').append(
        title('Установка'),
        infoBlock(
            subTitle('Во время демонстрационного режима работы библиотеки функциональность использования модулей временно недоступна. ( Попытки импортировать модули, например, с помощью синтаксиса import MC from "...", не дадут ожидаемых результатов в этот период. ) Так же MC поставляется отдельно от jQuery, учитите это в начале разработки.')
        ),
