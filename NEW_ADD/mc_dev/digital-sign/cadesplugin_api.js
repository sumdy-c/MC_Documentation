; (function () {
    ...
    function load_extension() {
        ...
        // Сначала пробуем динамически определить реальный ID установленного расширения,
        // затем падаем на hardcoded IDs из Chrome Web Store
        var dynamicDone = false;
        var loadHardcoded = function () {
            if (dynamicDone) return;
            load_js_script(manifestv2Url, nmcades_api_onload, function () {
                load_js_script(manifestv3Url, nmcades_api_onload, plugin_loaded_error);
            });
        };
        var dynamicTimer = setTimeout(loadHardcoded, 3000);
        get_extension_id(function (ext_id) {
            clearTimeout(dynamicTimer);
            if (ext_id) {
                var dynamicUrl = "chrome-extension://" + ext_id + "/nmcades_plugin_api.js";
                load_js_script(dynamicUrl, function () {
                    dynamicDone = true;
                    nmcades_api_onload();
                }, loadHardcoded);
            } else {
