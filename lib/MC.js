    ...
    if (arguments[0].prototype instanceof MC) {
      ...
      if (typeof arguments[1] === "string") {
        if (MC.keys.some((el_key) => el_key === arguments[1])) {
          console.error(
            `[MC] Обнаружено повторение ключа "${arguments[1]}" для компонента ${arguments[0].name}`
          );
        }
      } else {
        if (MC.keys.some((el_key) => el_key === arguments[2])) {
          console.error(
            `[MC] Обнаружено повторение ключа "${arguments[2]}" для компонента ${arguments[0].name}`
          );
        }
      }
      ...
      if (arg3) {
        ...
        if (!node) {
          ...
          try {
            ...
            console.error(
              "[MC] Попытка формирования элемента закончилась неудачно, возможно пытаетесь отдать под контроль элемент, который уже чем-то контролируется."
            );
          }
        ...
        if (!node) {
          ...
          try {
            ...
            console.error(
              "[MC] Попытка формирования элемента может закончится неудачей, возможно пытаетесь отдать под контроль элемент, который уже контролируется."
            );
          }
